...
class SideMenuContent extends React.Component {
  ...
  handleGroupClick = (id, parent = null, type = "") => {
    ...
    if (type === "collapse") {
      this.setState({ currentClicked: id });
  ...
  // Make child active if match url
  componentDidUpdate(prevProps, prevState) {
    navigationConfig.forEach((item) => {
      const open_group = this.state.activeGroups;
      if (window.location.href.includes(item.id) && !open_group.includes(item.id)) {
  ...
  render() {
    ...
    const menuItems = navigationConfig.map((item) => {
      ...
      // Displaying parent nav
      if (this.context.can("viewPage", item.id) || checkHasChild()) {
          ...
          <li
            className={classnames("nav-item", {
              ...
              active:
                ...
                (item.navLink &&
                  item.navLink !== "/" &&
                  window.location.href.includes(item.navLink)),
        ...
        if (
          ...
          (item.type === "item" && item.permissions === undefined)
        ) {
          return renderItem;
        }
        if (
...
SideMenuContent.contextType = AbilityContext;
